...
import minefantasy.item.mabShield.DamageSourceShieldfail;
...
public class CombatManager 
{
	...
	public static void onAttack(LivingHurtEvent event) 
	{
		...
		if (defense != null && !(event.source instanceof DamageSourceShieldfail))
		{
		...
		if (event.source instanceof EntityDamageSource && !(event.source instanceof EntityDamageSourceIndirect) && !event.source.damageType.equals("battlegearExtra") && !(event.source instanceof DamageSourceShieldfail) && !(event.source instanceof DamageSourceAP)) 
		{
			...
			if (entityHitter instanceof EntityLivingBase) 
			{
				...
				if (weapon != null) 
				{
					...
					if(weapon.getItem() instanceof IWeaponPenetrateArmour)
					{
						...
						damageWithBonus(target, event.source.getSourceOfDamage(), damage, dam);
									...
									for (int i = 0; i < player.inventory.armorInventory.length; i++) {
										...
										if (armorStack != null && armorStack.getItem() instanceof ItemArmor) {
											if (armorStack.getItem() instanceof ISpecialArmor) {
												if (armorStack.getItem() instanceof ItemArmourMF) {
													ItemArmourMF item = (ItemArmourMF) armorStack.getItem();
													if (item.getMaterial() == EnumArmourMF.GUILDED && ItemArmourMF.shouldSilverHurt(attacker)) {
														continue;
													} else if (item.getMaterial() == EnumArmourMF.IGNOTUMITE) {
														continue;
													} else if (item.getMaterial() == EnumArmourMF.DRAGONFORGE && event.source.isFireDamage()) {
														continue;
													}
												}
												((ISpecialArmor) armorStack.getItem()).damageArmor(player, armorStack, event.source, (int) armorDamage, i);
											} else {
												armorStack.damageItem((int) armorDamage, player);
											}
	 ...
	 */
	public static void damageWithBonus(EntityLivingBase target, Entity attacker, float damage, float bonus)
	{
		if(bonus < target.getHealth())
		{
			target.attackEntityFrom(new DamageSourceAP("blunt_mf", attacker), bonus+damage);
