...
public class MFProxy_common implements IGuiHandler {
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		...
		case 4:
			return new ContainerRack(player.inventory, (TileEntityWeaponRack) tile);
		case 6:
		...
		case 11:
			return new ContainerRoast(player.inventory, (TileEntityRoast) tile);

		case 12:
			return new ContainerFurnaceMF(player, player.inventory, (TileEntityFurnaceMF) tile);
		}
		return null;
	}
	...
	private Object getEntityContainer(EntityPlayer player, World world, Entity entity, int id) {
		if (entity instanceof EntityHound) {
			...
			if (id == 2) {
				return new ContainerPackHound(player.inventory, ((EntityHound) entity).pack,
						((EntityHound) entity).getAvailableRows());
			}
			if (id == 3) {
