...
public class TileEntityAnvilRenderer extends TileEntitySpecialRenderer {
	...
	public void renderAModelAt(TileEntityAnvil tile, double d, double d1, double d2, float f, int renderPass) {
		...
		GL11.glScalef(scale, -scale, -scale); // if you read this comment out
												// this line and you can see
												// what happens
		GL11.glPushMatrix();
		if (!tile.isBig()) {
		...
		for (int x = 0; x < mX; x++) {
			for (int y = 0; y < mY; y++) {
				...
				try {
					...
					if (cfg.renderWarnings) {
						Minecraft.getMinecraft().thePlayer.addChatMessage("Don't put that on the anvil");
					}
				}
	...
	private void renderBigItem(TileEntityAnvil tile, boolean move, ItemStack itemstack, double d, double d1,
			double d2) {
		GL11.glPushMatrix();
	...
	private boolean shouldRender(TileEntityAnvil tile, int p) {
		...
		if (p == 1)// GRID
		{
			return tile.worldObj != null
					&& (tile.gridTime > 0 || sp.getHeldItem() != null && sp.getHeldItem().getItem() instanceof ITongs);
		}
		return true;
	}
}