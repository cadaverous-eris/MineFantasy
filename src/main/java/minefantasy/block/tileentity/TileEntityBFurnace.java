...
public class TileEntityBFurnace extends TileEntity
	...
	public boolean structureExists() {
		if (blockMetadata != 1) // Only perform check for input block
			return true;
		if (!this.hasFuelBlock()) return false;
		//else System.out.println("Has Fuel");
		if (!this.hasResultBlock()) return false;
		//else System.out.println("Has Output");
			
		if (!isChamberBlock(xCoord, yCoord + 1, zCoord)) return false;
		if (!isChamberBlock(xCoord, yCoord + 2, zCoord)) return false;
		if (!isChamberBlock(xCoord, yCoord - 1, zCoord)) return false;
		if (!isChamberBlock(xCoord, yCoord - 2, zCoord)) return false;
		if (!isChamberBlock(xCoord, yCoord - 3, zCoord)) return false;
		if (!isChamberBlock(xCoord, yCoord - 4, zCoord)) return false;
