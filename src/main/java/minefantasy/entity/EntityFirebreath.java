...
public class EntityFirebreath extends Entity {
	...
	public EntityFirebreath poison() {
		dataWatcher.updateObject(4, 2);
		return this;
	}
	...
	public EntityFirebreath(World world, EntityLiving attacker, double xtar, double ytar, double ztar) {
		...
		this.setSize(1.0F, 1.0F);
		this.setLocationAndAngles(attacker.posX, attacker.posY, attacker.posZ, attacker.rotationYaw,
	...
	public void onUpdate() {
		...
		if (var15_1 > 0) {
			...
			AxisAlignedBB var2 = Block.blocksList[var15_1].getCollisionBoundingBoxFromPool(this.worldObj, this.xTile,
		...
		if (!this.worldObj.isRemote
			...
			if (canShoot(var4)) {
				...
				if (!worldObj.isRemote && rand.nextInt(3) == 0 && (!var4.isImmuneToFire() || getType() != 0)) {
			...
			if (this.worldObj.isRemote && getType() == 1) {
				int n = rand.nextInt(3) + 2;
				for (int i = 0; i < n; i++) {
					this.worldObj.spawnParticle("reddust", this.posX, this.posY, this.posZ, 0.45, 0.9, 0.95);
				}
			} else if (this.worldObj.isRemote && getType() == 2) {
				int n = rand.nextInt(3) + 2;
				for (int i = 0; i < n; i++) {
					this.worldObj.spawnParticle("reddust", this.posX, this.posY, this.posZ, 0.2, 0.95, 0.3);
				}
			}
			...
			if (this.isInWater()) {
				for (int var19 = 0; var19 < 4; ++var19) {
					float var18 = 0.25F;
					this.worldObj.spawnParticle("smoke", this.posX - this.motionX * (double) var18,
	...
	private void applyEffects(Entity tar) {
		if (tar != null && tar instanceof EntityLivingBase) {
			EntityLivingBase live = (EntityLivingBase) tar;
			if (getType() == 1) {
				// FROST EFFECT
				...
				live.addPotionEffect(new PotionEffect(Potion.poison.id, d, l));
			}
		}
	}
	...
	protected void hitEntity(MovingObjectPosition pos) {
		if (!this.worldObj.isRemote) {
			...
			if (pos.entityHit != null
					&& (pos.entityHit instanceof EntityDragonSmall || pos.entityHit instanceof EntityFirebreath)) {
				;
			} else
				setDead();
		}
	...
	private void freezeBlock(int x, int y, int z) {
		...
		if (repl == Block.waterStill.blockID)
			id = Block.ice.blockID;
		if (repl == Block.lavaStill.blockID)
			id = Block.obsidian.blockID;
		if (repl == Block.lavaMoving.blockID)
			id = Block.obsidian.blockID;
		if (worldObj.getBlockMaterial(x, y, z) == Material.leaves)
			id = BlockListMF.ice.blockID;
		if (worldObj.getBlockMaterial(x, y, z) == Material.plants)
			id = BlockListMF.ice.blockID;
	}

	private void placeBlock(int x, int y, int z, int id) {
		if (id > 0 && !worldObj.isAirBlock(x, y - 1, z))
