 ...
 * 
 *         Breeds: Red: 80Hp, high fire breath, high retreat Green: 120Hp, low
 *         fire breath, low retreat Black: 150Hp, Med fire breath, very high
 *         retreat, heals Gold: 200Hp, very high fire breath, very low retreat
 *         Purple: 100Hp, very high poison breath, very high retreat
 */
public class EntityDragonSmall extends EntityFlyingMob implements IMob {
	...
	public boolean attackEntityFrom(DamageSource source, int damage) {
		...
		if (source.getEntity() != null && source.getEntity() instanceof EntityPlayer) {
			if (getAttackTarget() == null || damage > 16
	...
	protected void damageEntity(DamageSource par1DamageSource, float par2) {
		...
		this.damageHealDelay = this.getDamageHealingDelay();
	}
	...
	protected void updateEntityActionState() {
		...
		if (this.targetedEntity != null && this.targetedEntity.getDistanceSqToEntity(this) < var9 * var9) {
			...
			boolean inRangeOfAttack = this.targetedEntity.getDistanceToEntity(this) < 4
					&& this.canEntityBeSeen(targetedEntity);
			boolean inRangeOfFire = this.targetedEntity.getDistanceToEntity(this) > 4
					&& this.targetedEntity.getDistanceToEntity(this) < 24 && fireBreathCooldown <= 0;
	...
	public void setEntityToAttack(Class enClass) {
		List list = worldObj.getEntitiesWithinAABB(enClass,
				AxisAlignedBB.getAABBPool().getAABB(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D)
	...
	private int getDamageHealingDelay() {
		switch (breed) {
		case 1:
			return 1200;
		case 2:
	...
	public void writeEntityToNBT(NBTTagCompound nbt) {
		...
		nbt.setInteger("healCooldown", this.healCooldown);
		nbt.setInteger("damageHealDelay", this.damageHealDelay);
	}
	...
	public void readEntityFromNBT(NBTTagCompound nbt) {
		...
		if (nbt.hasKey("healCooldown")) {
			this.healCooldown = nbt.getInteger("healCooldown");
		}
		if (nbt.hasKey("damageHealDelay")) {
			this.damageHealDelay = nbt.getInteger("damageHealDelay");
		}
	}
	...
	public String getTexture() {
		...
		case 5:
			return data_minefantasy.image("/mob/dragonPurple.png");
		}
		return data_minefantasy.image("/mob/dragonRed.png");
	}
	...
	public void onUpdate() {
		super.onUpdate();
		if (worldObj.getTotalWorldTime() < 2)
			setDead();
		if (ticksExisted == 1)
			createMount();
		if (!worldObj.isRemote) {
			if (damageHealDelay <= 0 && healCooldown <= 0) {
				heal(1);
				healCooldown = getHealCooldown();
			}
	...
	protected boolean canDespawn() {
		return this.worldObj.getClosestPlayerToEntity(this, 192) == null;
	}
	...
	protected void dropFewItems(boolean multiply, int amount) {
		this.dropSpecificItem(ItemListMF.misc.itemID, ItemListMF.fireGland, this.rand.nextInt(3) + 1);
	...
	public boolean getCanSpawnHere() {
		...
		// System.out.println("attempting dragon spawn at y = " + posY);
		...
		if ((dimension == 0 || dimension == -37)
				&& !worldObj.canBlockSeeTheSky((int) posX, (int) posY + 1, (int) posZ)) {
			return false;
		}
	...
	public boolean getCanWorldGenSpawnHere() {
		if (worldObj.difficultySetting < cfg.dragonDiff) {
			return false;
		}
		...
		for (Object o : worldObj.loadedEntityList) {
			if (o instanceof EntityDragonSmall) {
				// System.out.println(this.getDistanceToEntity((EntityDragonSmall)
				// o));
				if (this.getDistanceToEntity((EntityDragonSmall) o) < 192) {
					return false;
				}
			}
		}
		...
		int spawnDist = distanceToSpawn();
		int chance = (int) ((cfg.dragonChance / 4)
				- Math.min(((spawnDist - getDistanceToSpawn()) * 0.0025), cfg.dragonChance * 0.375));
		if (this.rand.nextInt(chance) != (chance - 1)) {
			return false;
		}
	...
	public int distanceToSpawn() {
		...
		return (int) getDistance(spawn.posX, spawn.posY, spawn.posZ);
	}
	...
	public boolean isInMountains() {
		...
		if (biome.biomeName.contains("Highlands") || biome.biomeName.contains("Mountains") || biome.biomeName.contains("Mesa") || biome.biomeName.contains("Cliffs")) {
			return true;
		}
		...
		if (!worldObj.isRemote) {
			for (Object o : worldObj.playerEntities) {
				if (o instanceof EntityPlayer) {
					...
					if (getDistanceToEntity(player) < 256 && getDistanceToEntity(player) > 16) {
						double distX = posX - player.posX;
						double distZ = posZ - player.posZ;
						String direction = "direction.";
						String message = "message.dragon_spawn_" + player.worldObj.rand.nextInt(4);
						boolean north = distZ <= 0;
						boolean west = distX <= 0;

						if (Math.abs(distX) > Math.abs(distZ)) {
							if (Math.abs(distZ) / Math.abs(distX) > 0.5) {
								direction += north ? "north" : "south";
							}
							direction += west ? "west" : "east";
						} else {
							direction += north ? "north" : "south";
							if (Math.abs(distX) / Math.abs(distZ) > 0.5) {
								direction += west ? "west" : "east";
							}
						}

						player.sendChatToPlayer(ChatMessageComponent.createFromTranslationWithSubstitutions(message,
								StatCollector.translateToLocal(direction)));
					}
				}
			}
		}
	...
	/*
	 * @Override public boolean getCanSpawnHere() { int chance = (this.dimension
	 * == 0 || this.dimension == -37) ? cfg.dragonChance : cfg.dragonChance / 4;
	 * if (rand.nextInt(cfg.dragonChance) != 0) { return false; }
	 * 
	 * if (worldObj.difficultySetting < cfg.dragonDiff) { return false; }
	 * 
	 * if (!super.getCanSpawnHere()) { return false; } if (!canSpawnAtPos()) {
	 * return false; } if (MineFantasyBase.isDebug()) {
	...
	public boolean canSpawnAtPos() {
		if (this.dimension != 0 && this.dimension != -37) {
			return true;
		}
		ChunkCoordinates spawn = worldObj.getSpawnPoint();
				...
				for (int z = var4; z <= var7; ++z) {
					...
					if (var13 != null) {
						if (var13 == Material.glass) {
							...
							if (Block.blocksList[id].getBlockHardness(worldObj, x, y, z) < 50F
									&& Block.blocksList[id].getBlockHardness(worldObj, x, y, z) > 0F) {
								var9 = true;
								this.worldObj.setBlockToAir(x, y, z);
								Block.blocksList[id].dropBlockAsItem(worldObj, x, y, z, meta, 0);
								for (int a = 0; a < 1 + rand.nextInt(4); a++)
				...
				for (int var12 = var4; var12 <= var7; ++var12) {
					...
					if (var13 != null) {
						if (var13 == Material.glass || isMaterialBreakable(var13)) {
							var9 = true;
							// this.worldObj.setBlockWithNotify(var10, var11,
							// var12, 0);
							return true;
						} else {
							var8 = true;
	...
	private boolean isMaterialBreakable(Material material) {
		return material == Material.cactus || material == Material.cake || material == Material.circuits
				|| material == Material.cloth || material == Material.leaves || material == Material.pumpkin
				|| material == Material.redstoneLight || material == Material.plants;
	}
