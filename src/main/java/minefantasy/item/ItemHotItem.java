...
public class ItemHotItem extends Item implements IHotItem {
	...
	public static ItemStack getItem(ItemStack item) {
		...
		if (tag.hasKey("ingotID") && tag.hasKey("ingotMeta")) {
			ItemStack stack = new ItemStack(tag.getInteger("ingotID"), 1, tag.getInteger("ingotMeta"));
			if (tag.hasKey("ingotTag")) {
				stack.stackTagCompound = tag.getCompoundTag("ingotTag");
			}
			return stack;
		}
	...
	public static ItemStack createHotItem(ItemStack item, boolean display) {
		...
		if (item.hasTagCompound()) {
			nbt.setCompoundTag("ingotTag", item.stackTagCompound);
		}
	...
	/**
	 * Colour shading Half 1: 255,255,255 - 255,0,0
	 * 
	 * Half 2: 255,0,0 - 255,255,0
	 */
