 ...
 */
public class ItemArmourMFOld extends ItemArmor implements
	...
	{
		if (getMaterial() == MedievalArmourMaterial.DRAGON)
		{
	...
	public EnumArmourClass getArmourClass() {
		if (getMaterial() == MedievalArmourMaterial.APRON || getMaterial() == MedievalArmourMaterial.RAWHIDE || getMaterial() == MedievalArmourMaterial.STEALTH)return EnumArmourClass.LIGHT;
	...
	public float fireResistance() {
		...
		if (getMaterial() == MedievalArmourMaterial.APRON) {
			return 0.8F;
		}
		return 1.0F;
	}
	...
	public float arcaneResistance() {
		if (getMaterial() == MedievalArmourMaterial.ENCRUSTED) {
			return 0.8F;
		}
		return 1.0F;
	}
	...
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot)
	{
		...
		else if(source == DamageSourceAP.blunt || source instanceof DamageSourceAP)
		{
			AC *= AD.bluntResist;
		}
		else
		...
		if(source != DamageSourceAP.blunt && !(source instanceof DamageSourceAP) && source.isUnblockable())
		{
			AC = 0;
		}
	...
	public int getArmorDisplay(EntityPlayer player, ItemStack armor, int slot) {
		float AC = (float)(((ItemArmor)armor.getItem()).damageReduceAmount);
		AC *= AD.protection;
	...
	{
		if(getMaterial() == MedievalArmourMaterial.GUILDED && source != null && source.getEntity() != null)
		{
			if(source.getEntity() instanceof EntityLivingBase)
			{
		...
		if(getMaterial() == MedievalArmourMaterial.DRAGON && source != null && source.isFireDamage())
		{
			return;
		}
	...
	public float getMoveSpeed(ItemStack item) {
		...
		if(this.getMaterial() == MedievalArmourMaterial.MITHRIL)
		{
			decr /= 2;
		}
		...
		if (getMaterial() == MedievalArmourMaterial.APRON || getMaterial() == MedievalArmourMaterial.RAWHIDE)return 1.0F;
