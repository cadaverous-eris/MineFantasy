...
public class ItemShield extends Item
	...
	public void registerIcons(IconRegister reg) {
		...
		backIcon = reg.registerIcon(
				"minefantasy:Shield/" + design.getTitle() + "/" + design.getTitle() + "." + getMatName() + ".back");
		trimIcon = reg.registerIcon(
				"minefantasy:Shield/" + design.getTitle() + "/" + design.getTitle() + "." + getMatName() + ".trim");
	...
	public float getDecayRate(ItemStack shield) {
		int use = 0;
		for (Object e : EnchantmentHelper.getEnchantments(shield).entrySet()) {
			if (e != null && e instanceof Entry) {
				Entry entry = (Entry) e;
				if (entry.getKey() != null && entry.getKey() instanceof Integer) {
					int id = (Integer) entry.getKey();
					if (Enchantment.enchantmentsList[id] != null && Enchantment.enchantmentsList[id].getName().contains("shield.usage")) {
						if (entry.getValue() != null && entry.getValue() instanceof Integer) {
							use = (Integer) entry.getValue();
							break;
						}
					}
				}
	...
	public float getRecoveryRate(ItemStack shield) {
		int recover = 0;
		//EnchantmentHelper.getEnchantmentLevel(BaseEnchantment.shieldRecover.effectId, shield);
		for (Object e : EnchantmentHelper.getEnchantments(shield).entrySet()) {
			if (e != null && e instanceof Entry) {
				Entry entry = (Entry) e;
				if (entry.getKey() != null && entry.getKey() instanceof Integer) {
					int id = (Integer) entry.getKey();
					if (Enchantment.enchantmentsList[id] != null && Enchantment.enchantmentsList[id].getName().contains("shield.recover")) {
						if (entry.getValue() != null && entry.getValue() instanceof Integer) {
							recover = (Integer) entry.getValue();
							break;
						}
					}
				}
	...
	public boolean canBlock(ItemStack shield, DamageSource source) {
		...
		if (source == DamageSourceAP.blunt || source instanceof DamageSourceAP) {
	...
	public void addInformation(ItemStack item, EntityPlayer player, List list, boolean extra) {
		...
		if (MineFantasyBase.isBGLoaded() || MineFantasyBase.isDebug()) {
			...
			if (arrowCount > 0) {
				list.add(String.format("%s%s %s", EnumChatFormatting.GOLD, arrowCount,
	...
	public boolean hasColor(ItemStack item) {
		return item.hasTagCompound() && item.getTagCompound().hasKey("display")
				&& item.getTagCompound().getCompoundTag("display").hasKey("color");
	}
	...
	public int getColor(ItemStack item) {
		{
			...
			if (nbttagcompound == null) {
				...
				return nbttagcompound1 == null ? getDefaultColor(item)
	...
	public float getThreshold(ItemStack item, DamageSource src) {
		float mp = 1.0F;
		if (src != null) {
			if (design == EnumShieldDesign.TOWER && (src == DamageSourceAP.blunt || src instanceof DamageSourceAP)) {
				mp = 0.25F;
			}
		}
	...
	public void getSubItems(int id, CreativeTabs tabs, List list) {
		...
		add(list, ItemListMF.bucklerIgnotumite);
		...
		add(list, ItemListMF.kiteIgnotumite);
		...
		add(list, ItemListMF.towerIgnotumite);
	}
	...
	public boolean onHit(ItemStack shield, EntityLivingBase defender, DamageSource source, EntityLivingBase attacker,
			float dam) {
